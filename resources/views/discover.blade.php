...
<head>
    ...
    <title>Blokpedia - Cari Koin</title>
    <link rel="icon" type="image/png" href="{{ asset('logo-blokpedia.png') }}">
    <script src="https://cdn.tailwindcss.com"></script>
    <style> 
        body { background-color: #0b0e14; color: #e2e8f0; } 
        .ds-bg-panel { background-color: #151924; }
        .ds-border { border-color: #1e2433; }
        .ds-text-green { color: #20d981; }
        .ds-bg-green { background-color: #20d981; }
...
<body class="p-4 md:p-8 font-sans relative min-h-screen">
    ...
    <div class="max-w-7xl mx-auto ds-bg-panel p-6 rounded-xl border ds-border shadow-2xl">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center border-b ds-border pb-5 mb-6 gap-4">
            ...
            <a href="/" class="bg-gray-800 hover:bg-gray-700 border ds-border text-white py-2 px-5 rounded-lg font-bold transition duration-200 flex items-center gap-2">
                <span>⬅</span> Kembali ke Dashboard
            </a>
        </div>

        <form action="/discover" method="GET" class="flex flex-col md:flex-row gap-3 mb-8" onsubmit="showLoading()">
            <input type="text" name="q" value="{{ $keyword ?? '' }}" placeholder="Cari koin (Contoh: pepe, shiba, atau paste Smart Contract)..." 
            ...
            <button type="submit" class="ds-bg-green hover:bg-[#1bb86d] text-black px-8 py-4 rounded-lg font-black text-base transition shadow-[0_0_15px_rgba(32,217,129,0.2)] uppercase tracking-wider">
                Cari Koin
            </button>
        </form>
                        ...
                        @foreach($pairs as $index => $pair)
                            <tr class="hover:bg-[#151924] transition duration-150">
                                ...
                                <td class="p-4">
                                    <div class="flex items-center gap-3">
                                        @if(isset($pair['info']['imageUrl']))
                                            <img src="{{ $pair['info']['imageUrl'] }}" class="w-10 h-10 rounded-full border ds-border object-cover" alt="Logo">
                                        @else
                                            <div class="w-10 h-10 rounded-full ds-bg-panel border ds-border flex items-center justify-center text-sm font-black text-white">
                                                {{ substr($pair['baseToken']['symbol'], 0, 1) }}
                                            </div>
                                        @endif
                                        
                                        <div>
                                            <div class="font-bold text-base text-white">{{ $pair['baseToken']['name'] }}</div>
                                            <div class="text-xs text-gray-400 font-mono mt-0.5">
                                                {{ $pair['baseToken']['symbol'] }} 
                                                <span class="text-gray-600">/ {{ $pair['quoteToken']['symbol'] ?? 'SOL' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                @php
                                    ...
                                    $isUp = $priceChange >= 0;
                                @endphp
                                <td class="p-4 font-bold text-right text-sm {{ $isUp ? 'ds-text-green' : 'text-red-500' }}">
                                    {{ $isUp ? '+' : '' }}{{ number_format($priceChange, 2) }}%
                                </td>
                                
                                <td class="p-4 text-center">
                                    <span class="bg-blue-900/30 text-blue-400 py-1.5 px-3 rounded text-[10px] font-black uppercase tracking-wider border border-blue-800">
                                        {{ $pair['chainId'] }}
                                    </span>
                                </td>
                                
                                <td class="p-4 text-right">
                                    <form action="/add-coin" method="POST" class="m-0 inline-block" onsubmit="showLoading()">
                                        @csrf
                                        <input type="hidden" name="name" value="{{ $pair['baseToken']['name'] }}">
                                        <input type="hidden" name="symbol" value="{{ $pair['baseToken']['symbol'] }}">
                                        <input type="hidden" name="token_address" value="{{ $pair['baseToken']['address'] }}">
                                        <input type="hidden" name="chain_id" value="{{ $pair['chainId'] }}">
                                        <input type="hidden" name="pair_address" value="{{ $pair['pairAddress'] }}">
                                        
                                        <button type="submit" class="bg-[#1e2433] hover:bg-[#2d3748] border ds-border text-white px-4 py-2 rounded font-bold text-xs transition uppercase tracking-wider">
                                            + Tambah Layar
                                        </button>
                                    </form>
                                </td>
